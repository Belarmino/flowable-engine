...
angular.module('flowableModeler')
  .controller('ReportCtrl', ['$rootScope', '$scope', '$translate', '$http', '$location', '$routeParams','$modal', '$timeout', '$popover', 
    ...
    $scope.showVersion = function(version) {
      if (version) {
        if (version.latestVersion) {
            $location.path("/reports/" +  $scope.model.latestModelId);
        } else {
          // Show latest version, no history-suffix needed in URL
          $location.path("/reports/" +  $scope.model.latestModelId + "/history/" + version.id);
        }
      }
    };
    ...
    $scope.duplicateForm = function() {
      ...
      var modalInstance = _internalCreateModal({
        template: 'views/popup/report-duplicate.html?version=' + Date.now()
      }, $modal, $scope);
    ...
    $scope.openEditor = function() {
      if ($scope.model.form) {
    	  $location.path("/report-editor/" + $scope.model.form.id);
      }
    };
