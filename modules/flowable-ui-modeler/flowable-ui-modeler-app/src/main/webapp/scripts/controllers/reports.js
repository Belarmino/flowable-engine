...
angular.module('flowableModeler')
  .controller('ReportsCtrl', ['$rootScope', '$scope', '$translate', '$http', '$timeout','$location', '$modal', function ($rootScope, $scope, $translate, $http, $timeout, $location, $modal) {

      // Main page (needed for visual indicator of current page)
      $rootScope.setMainPageById('reports');
	  ...
	  $scope.createForm = function() {
		  ...
		  $scope.createFormCallback = function(result) {
			  ...
			  $location.path("/report-editor/" + result.id);
		  };
          _internalCreateModal({
			  template: 'views/popup/report-create.html?version=' + Date.now(),
			  scope: $scope
		  }, $modal, $scope);
	  };

	  $scope.showFormDetails = function(report) {
	      if (report) {
	          $rootScope.editorHistory = [];
	  ...
	  $scope.editFormDetails = function(report) {
		  if (report) {
		      $rootScope.editorHistory = [];
			  $location.path("/report-editor/" + report.id);
		  }
	  };
...
angular.module('flowableModeler')
.controller('CreateNewReportCtrl', ['$rootScope', '$scope', '$http',
    ...
    $scope.ok = function () {
        ...
        if (!$scope.model.report.name || $scope.model.report.name.length == 0 ||
        	!$scope.model.report.key || $scope.model.report.key.length == 0) {
        ...
        $http({method: 'POST', url: FLOWABLE.CONFIG.contextRoot + '/app/rest/models', data: $scope.model.report}).
            success(function(data, status, headers, config) {
                $scope.$hide();
                $scope.model.loading = false;
...
angular.module('flowableModeler')
	.controller('DuplicateReportCtrl', ['$rootScope', '$scope', '$http',
		function ($rootScope, $scope, $http) {
			...
			if ($scope.originalModel) {
				//clone the model
				$scope.model.report.name = $scope.originalModel.report.name;
				$scope.model.report.key = $scope.originalModel.report.key;
				$scope.model.report.description = $scope.originalModel.report.description;
				$scope.model.report.modelType = $scope.originalModel.report.modelType;
				$scope.model.report.id = $scope.originalModel.report.id;
			}
			...
			$scope.ok = function () {
				...
				$http({method: 'POST', url: FLOWABLE.CONFIG.contextRoot + '/app/rest/models/'+$scope.model.report.id+'/clone', data: $scope.model.report}).
					success(function(data, status, headers, config) {
						$scope.$hide();
						$scope.model.loading = false;
